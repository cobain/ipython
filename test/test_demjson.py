...
class DemjsonTest(unittest.TestCase):
    ...
    def testDecodeLargeIntegers(self):
        ...
        self.assertEqual(demjson.decode('-9876543210123456789'), -9876543210123456789)
        self.assertEqual(demjson.decode('0xfedcba9876543210ABCDEF', allow_hex_numbers=True),
                         308109520888805757320678895)
        self.assertEqual(demjson.decode('-0xfedcba9876543210ABCDEF', allow_hex_numbers=True),
                         -308109520888805757320678895)
        self.assertEqual(
            demjson.decode('0177334565141662503102052746757', allow_leading_zeros=True, leading_zero_radix=8),
            308109520888805757320678895)
        self.assertEqual(
            demjson.decode('-0177334565141662503102052746757', allow_leading_zeros=True, leading_zero_radix=8),
            -308109520888805757320678895)
    ...
    def testDecodeOctalNumbers(self):
        ...
        self.assertRaises(demjson.JSONDecodeError, demjson.decode, '018', allow_leading_zeros=True,
    ...
    def testDecodeNaN(self):
        ...
        self.assertTrue(is_nan(demjson.decode('-NaN', allow_non_numbers=True)))
        if decimal:
            self.assertTrue(isinstance(demjson.decode('NaN', allow_non_numbers=True, float_type=demjson.NUMBER_DECIMAL),
                                       decimal.Decimal))
    ...
    def testDecodeInfinite(self):
        ...
        if decimal:
            self.assertTrue(
                isinstance(demjson.decode('Infinity', allow_non_numbers=True, float_type=demjson.NUMBER_DECIMAL),
                           decimal.Decimal))
            self.assertTrue(
                is_infinity(demjson.decode('Infinity', allow_non_numbers=True, float_type=demjson.NUMBER_DECIMAL)))
            self.assertTrue(
                is_infinity(demjson.decode('+Infinity', allow_non_numbers=True, float_type=demjson.NUMBER_DECIMAL)))
            self.assertTrue(
                is_infinity(demjson.decode('-Infinity', allow_non_numbers=True, float_type=demjson.NUMBER_DECIMAL)))
            self.assertTrue(
                demjson.decode('-Infinity', allow_non_numbers=True, float_type=demjson.NUMBER_DECIMAL).is_signed())
    ...
    def testEncodeHexIntegers(self):
        ...
        self.assertEqual(demjson.encode(demjson.json_int(0x123456789abcdef, number_format=demjson.NUMBER_FORMAT_HEX)),
                         '0x123456789abcdef')
    ...
    def testEncodeLegacyOctalIntegers(self):
        ...
        self.assertEqual(demjson.encode(demjson.json_int(-359, number_format=demjson.NUMBER_FORMAT_LEGACYOCTAL)),
                         '-0547')
    ...
    def testIntAsFloat(self):
        ...
        if decimal:
            self.assertEqual(demjson.decode('[0,-5,600,0xFF]', int_as_float=True, float_type=demjson.NUMBER_DECIMAL),
                             [decimal.Decimal('0.0'), decimal.Decimal('-5.0'), decimal.Decimal('600.0'),
                              decimal.Decimal('255.0')])
            self.assertEqual([type(x) for x in
                              demjson.decode('[0,-5,600,0xFF]', int_as_float=True, float_type=demjson.NUMBER_DECIMAL)],
    ...
    def testDecodeWithISO8859_1(self):
        ...
        if have_iso8859_1:
            # Try ISO-8859-1
            d = rawbytes(
                [ord('"'), ord('a'), 0xe0, ord('\\'), ord('u'), ord('0'), ord('1'), ord('5'), ord('2'), ord('"')])
    ...
    def testEncodeWithISO8859_1(self):
        ...
        if have_iso8859_1:
            ...
            self.assertEqual(demjson.encode(s, encoding='iso8859-1'),
                             rawbytes(
                                 [ord('"'), ord('a'), 0xe0, ord('\\'), ord('u'), ord('0'), ord('1'), ord('5'), ord('2'),
                                  ord('"')]))
    ...
    def testDecodeObjectWithDuplicates(self):
        ...
        self.assertRaises(demjson.JSONDecodeError, demjson.decode, '{"b":9,"a":1,"c":42,"a":2}',
    ...
    def testStatsIntegers(self):
        ...
        n64s = [2147483648, 9223372036854775807, -2147483649,
        ...
        n53s = [-9007199254740992, -9007199254740991, 9007199254740991,
    ...
    def testStatsFloats(self):
        ...
        if decimal:
            ...
            self.assertEqual(self.decode_stats(
                '3.141592653589793238462643383279502884197169399375105820974944592307816406286').num_floats_decimal, 1)
